    ...
    options: [
      {
        ...
        type: 1,
        options: [
          { name: "channel", description: "Channel for tickets", type: 7, required: true },
          ...
          { name: "image", description: "Panel image", type: 3, required: false },
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (sub === "list") {
        ...
        const lines = entries.map((p) => `• \`${p.panelId}\` — ${p.title} — <#${p.channelId}>`);
    ...
    if (interaction.isButton()) {
      ...
      if (action === "open_ticket") {
        ...
        const embed = new EmbedBuilder().setTitle(panel.title).setDescription(panel.description).setColor(0x57f287);
